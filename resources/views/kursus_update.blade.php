...
<form method="POST" action="/kursus/update/{{ $data->id }}" enctype="multipart/form-data"
    ...
    <div class="bg-white  shadow-xl rounded px-8 pt-6 pb-8 mb-4 flex flex-col my-2 w-10/12 mx-auto">
        ...
        <div class="-mx-3 md:flex mb-6">
            <div class="md:w-1/2 px-3 mb-6 md:mb-0">
                ...
                <input name="nama_kursus" value="{{ $data->nama_kursus }}"
        ...
        <div class="-mx-3 md:flex mb-6">
            ...
            {{-- <div class="md:w-1/3 px-3">
                ...
                <select id="countries" name="we"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">

                    @foreach ($pengajar as $dat)
                        @if ($dat->id == $data->pengajar_id)
            ...
            <div class="md:w-1/3 px-3">
                ...
                <select id="countries" name="category_id"
                    ...
                    @foreach ($category as $dat)
                        @if ($dat->id == $data->category_id)
                            <option value="{{ $dat->id }}" selected>{{ $dat->nama_pelajaran }}</option>
                        @else
                            <option value="{{ $dat->id }}">{{ $dat->nama_pelajaran }}</option>
                        @endif
                    @endforeach
                </select>
            </div>
        ...
        <div class="-mx-3 md:flex mb-6">
            <div class="md:w-1/2 px-3 mb-6 md:mb-0">
                ...
                <textarea name="deskripsi"
                    class="appearance-none block w-full bg-grey-lighter text-grey-darker border border-red rounded py-3 px-4 mb-3"
                    id="grid-first-name" type="text" placeholder="Masukkan Deskripsi . . .">{{ $data->deskripsi }}</textarea>
                <p class="text-red text-xs italic">Please fill out this field.</p>
            </div>
